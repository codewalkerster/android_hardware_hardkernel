...
#ifdef VIDEO_SNAPSHOT
    SnapshotBuffer[0] = NULL;
    SnapshotBuffer[1] = NULL;
#endif
...
void CameraHardwareSec::initDefaultParameters(int cameraId)
{
    ...
    mUseInternalISP = mSecCamera->getUseInternalISP();
        ...
        if (mUseInternalISP) {
            ...
            //M5MO
            p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES,
                  "3264x2448,1920x1080,1280x720,800x480,720x480,640x480,352x288,320x240,528x432,176x144");
    ...
    if (cameraId == SecCamera::CAMERA_ID_BACK) {
        ...
        p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES, "7,15,20,30");
    ...
    if (!mUseInternalISP) {
        ...
        p.set(CameraParameters::KEY_MAX_ZOOM, 31 - 1);
	p.set(CameraParameters::KEY_ZOOM_RATIOS, "31,40");
    }
...
#else
    if (w->set_usage(w, GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_HWC_HWOVERLAY)) {
        ALOGE("%s: could not set usage on gralloc buffer", __func__);
        return INVALID_OPERATION;
    }
#endif
...
int CameraHardwareSec::previewThread()
{
    ...
    if (mPreviewWindow && mGrallocHal && mPreviewRunning) {
	    ...
	    virAddr[1] = virAddr[0] + (GRALLOC_ALIGN(width, 16) * height);
	    virAddr[2] = virAddr[1] + ((GRALLOC_ALIGN(width, 16) * height) / 4);
	    ...
	    if (mPreviewFmtPlane == PREVIEW_FMT_1_PLANE) {
		// YCbCr
		memcpy(virAddr[0],src, width * height * 2);
	    } else {
			// Y
			memcpy(virAddr[0], previewAddr.virt.extP[0], width * height);

			if (mPreviewFmtPlane == PREVIEW_FMT_2_PLANE) {
				// CbCr
				memcpy(virAddr[1], previewAddr.virt.extP[1], width * height / 2);
			} else if (mPreviewFmtPlane == PREVIEW_FMT_3_PLANE) {
				// Cb
				memcpy(virAddr[1], previewAddr.virt.extP[1], width * height / 4);
				// Cr
				memcpy(virAddr[2], previewAddr.virt.extP[2], width * height / 4);
			}
	    }
...
#ifdef VIDEO_SNAPSHOT
	if (mVideoSnapshot == true) {
		memcpy(SnapshotBuffer[0], recordAddr.virt.extP[0], mRecordWidth * mRecordHeight);
		memcpy(SnapshotBuffer[1], recordAddr.virt.extP[1], (mRecordWidth * mRecordHeight) / 2);
		mVideoSnapshot = false;
		mbufferCopied.signal();
	}
#endif

        ALOGV("record VirtY(0x%08x) VirtC(0x%08x) ", recordAddr.virt.extP[0], recordAddr.virt.extP[1]);

        addrs = (struct addrs *)(*mRecordHeap)->data;

        addrs[index].type   = kMetadataBufferTypeCameraSource;
        addrs[index].addr_y = recordAddr.virt.extP[0];
        addrs[index].addr_cbcr = recordAddr.virt.extP[1];
...
#ifdef VIDEO_SNAPSHOT
    if (mRecordRunning) {
	mSecCamera->getRecordingSize(&mRecordWidth, &mRecordHeight);
	SnapshotBuffer[0] = (void *) malloc(mRecordWidth * mRecordHeight);
	SnapshotBuffer[1] = (void *) malloc((mRecordWidth * mRecordHeight) / 2);
	mVideoSnapshot = true;
    }
#endif
...
again:
			...
			/* Following code sends jpeg heap filled with zero.
	                   This is done to start the preview again if fimc_poll
	                   times out while capturing an image. */
	                camera_memory_t *JpegHeap_out = mGetMemoryCb(-1, 100, 1, 0);
	                memset(JpegHeap_out->data, 0, 100);
		...
		scaleDownYuv422((char *)mCapBuffer.virt.extP[0], cap_width, cap_height,
			...
			/* Wait only if it is not yet signaled, otherwise it will
			   result in infinite wait */
			if (mVideoSnapshot != false)
				mbufferCopied.wait(mSnapshotLock);

			if (mSecCamera->getVideoSnapshotAndJpeg(SnapshotBuffer,
					(unsigned char*)JpegHeap->data, &JpegImageSize) < 0) {
	                        mStateLock.lock();
	                        mCaptureInProgress = false;
	                        mStateLock.unlock();
...
out:
	...
	free(SnapshotBuffer[0]);
	free(SnapshotBuffer[1]);
	SnapshotBuffer[0] = NULL;
	SnapshotBuffer[1] = NULL;
    }
#endif
...
status_t CameraHardwareSec::setParameters(const CameraParameters& params)
{
    ...
    if (new_record_hint_str) {
        if (strncmp(new_record_hint_str, curr_record_hint_str, 5)) {
            ...
            if (mUseInternalISP && mSecCamera->setMode(mRecordHint) < 0) {
                ALOGV("ERR(%s):fail on mSecCamera->setMode(%d)", __func__, mRecordHint);
                ret = UNKNOWN_ERROR;
            } else {
            ...
            if (mUseInternalISP) {
                if (mSecCamera->initSetParams() < 0) {
                    ALOGV("ERR(%s):fail on mSecCamera->initSetParams()", __func__);
                    ret = UNKNOWN_ERROR;
                }
            }
        }
    ...
    if (0 < new_preview_width && 0 < new_preview_height &&
        ...
        if (!strcmp(new_str_preview_format,
            ...
            mFrameSizeDelta = 0;
                         ...
                         CameraParameters::PIXEL_FORMAT_YUV420SP)) {
            new_preview_format = V4L2_PIX_FMT_NV12M;
            mPreviewFmtPlane = PREVIEW_FMT_2_PLANE;
        }
        else if (!strcmp(new_str_preview_format,
                         CameraParameters::PIXEL_FORMAT_YUV420P)) {
            new_preview_format = V4L2_PIX_FMT_YUYV;
            mPreviewFmtPlane = PREVIEW_FMT_1_PLANE;
        }
    ...
    if (new_iso_str != NULL) {
        ...
        if (!strcmp(new_iso_str, "auto")) {
            ...
            ALOGV("ERR(%s):Invalid iso value(%s)", __func__, new_iso_str);
            ret = UNKNOWN_ERROR;
        }

        if (0 <= new_iso) {
            if (mSecCamera->setISO(new_iso) < 0) {
                ALOGV("ERR(%s):Fail on mSecCamera->setISO(iso(%d))", __func__, new_iso);
                ret = UNKNOWN_ERROR;
            } else {
    ...
    if (new_metering_str != NULL) {
        ...
        if (!strcmp(new_metering_str, "center")) {
            ...
            ALOGV("ERR(%s):Invalid metering value(%s)", __func__, new_metering_str);
            ret = UNKNOWN_ERROR;
        }

        if (0 <= new_metering) {
            if (mSecCamera->setMetering(new_metering) < 0) {
                ALOGV("ERR(%s):Fail on mSecCamera->setMetering(metering(%d))", __func__, new_metering);
                ret = UNKNOWN_ERROR;
            } else {
    ...
    if (new_antibanding_str != NULL) {
        ...
        if (!strcmp(new_antibanding_str, CameraParameters::ANTIBANDING_AUTO)) {
            ...
            ALOGV("ERR(%s):Invalid antibanding value(%s)", __func__, new_antibanding_str);
            ret = UNKNOWN_ERROR;
        }

        if (0 <= new_antibanding) {
            if (mSecCamera->setAntiBanding(new_antibanding) < 0) {
                ALOGV("ERR(%s):Fail on mSecCamera->setAntiBanding(antibanding(%d))", __func__, new_antibanding);
                ret = UNKNOWN_ERROR;
            } else {
    ...
    if ((new_scene_mode_str != NULL) && (current_scene_mode_str != NULL) && strncmp(new_scene_mode_str, current_scene_mode_str, 5)) {
        ...
        if (!strcmp(new_scene_mode_str, CameraParameters::SCENE_MODE_AUTO)) {
            new_scene_mode = V4L2_SCENE_MODE_NONE;
        } else {
            ...
            if (!strcmp(new_scene_mode_str, CameraParameters::SCENE_MODE_PORTRAIT)) {
                new_scene_mode = V4L2_SCENE_MODE_PORTRAIT;
                if (mCameraID == SecCamera::CAMERA_ID_BACK)
                ...
                new_scene_mode = V4L2_SCENE_MODE_BEACH_SNOW;
            } else if (!strcmp(new_scene_mode_str, CameraParameters::SCENE_MODE_SUNSET)) {
                new_scene_mode = V4L2_SCENE_MODE_SUNSET;
                ...
                new_scene_mode = V4L2_SCENE_MODE_FIREWORKS;
            } else if (!strcmp(new_scene_mode_str, CameraParameters::SCENE_MODE_CANDLELIGHT)) {
                new_scene_mode = V4L2_SCENE_MODE_CANDLE_LIGHT;
            } else {
    ...
    if ((new_focus_mode_str != NULL) && (mCameraID == SecCamera::CAMERA_ID_BACK)) {
        ...
        if (!strcmp(new_focus_mode_str,
            ...
            ALOGV("%s::unmatched focus_mode(%s)", __func__, new_focus_mode_str);
            ret = UNKNOWN_ERROR;
        }

        if (0 <= new_focus_mode) {
            if (mSecCamera->setFocusMode(new_focus_mode) < 0) {
                ALOGV("%s::mSecCamera->setFocusMode(%d) fail", __func__, new_focus_mode);
                ret = UNKNOWN_ERROR;
            } else {
    ...
    if (new_flash_mode_str != NULL) {
        ...
        if (!strcmp(new_flash_mode_str, CameraParameters::FLASH_MODE_OFF))
            ...
            ALOGV("%s::unmatched flash_mode(%s)", __func__, new_flash_mode_str); //red-eye
            ret = UNKNOWN_ERROR;
        }
        if (0 <= new_flash_mode) {
            if (mSecCamera->setFlashMode(new_flash_mode) < 0) {
                ALOGV("%s::mSecCamera->setFlashMode(%d) fail", __func__, new_flash_mode);
                ret = UNKNOWN_ERROR;
            } else {
    ...
    if ((new_scene_mode_str != NULL) && !strcmp(new_scene_mode_str, CameraParameters::SCENE_MODE_AUTO)) {
        if (new_white_str != NULL) {
            ...
            if (!strcmp(new_white_str, CameraParameters::WHITE_BALANCE_AUTO)) {
                ...
                ALOGV("ERR(%s):Invalid white balance(%s)", __func__, new_white_str); //twilight, shade, warm_flourescent
                ret = UNKNOWN_ERROR;
            }

            if (0 <= new_white) {
                if (mSecCamera->setWhiteBalance(new_white) < 0) {
                    ALOGV("ERR(%s):Fail on mSecCamera->setWhiteBalance(white(%d))", __func__, new_white);
                    ret = UNKNOWN_ERROR;
                } else {
                    mParameters.set(CameraParameters::KEY_WHITE_BALANCE, new_white_str);
                }
    ...
    if (new_image_effect_str != NULL) {
        ...
        if (!strcmp(new_image_effect_str, CameraParameters::EFFECT_NONE)) {
            ...
            ALOGV("ERR(%s):Invalid effect(%s)", __func__, new_image_effect_str);
            ret = UNKNOWN_ERROR;
        }

        if (new_image_effect >= 0) {
            if (mSecCamera->setImageEffect(new_image_effect) < 0) {
                ALOGV("ERR(%s):Fail on mSecCamera->setImageEffect(effect(%d))", __func__, new_image_effect);
                ret = UNKNOWN_ERROR;
            } else {
    ...
    if (new_contrast_str != NULL) {
        ...
        if (!strcmp(new_contrast_str, "auto")) {
            ...
            ALOGV("ERR(%s):Invalid contrast value(%s)", __func__, new_contrast_str);
            ret = UNKNOWN_ERROR;
        }

        if (0 <= new_contrast) {
            if (mSecCamera->setContrast(new_contrast) < 0) {
                ALOGV("ERR(%s):Fail on mSecCamera->setContrast(contrast(%d))", __func__, new_contrast);
                ret = UNKNOWN_ERROR;
            } else {
    ...
    if (new_gamma_str != NULL) {
        ...
        if (!strcmp(new_gamma_str, "off"))
            ...
            ALOGV("%s::unmatched gamma(%s)", __func__, new_gamma_str);
            ret = UNKNOWN_ERROR;
        }

        if (0 <= new_gamma) {
            if (mSecCamera->setGamma(new_gamma) < 0) {
                ALOGV("%s::mSecCamera->setGamma(%d) fail", __func__, new_gamma);
                ret = UNKNOWN_ERROR;
            }
        }
    }
    ...
    if (new_slow_ae_str != NULL) {
        ...
        if (!strcmp(new_slow_ae_str, "off"))
            ...
            ALOGV("%s::unmatched slow_ae(%s)", __func__, new_slow_ae_str);
            ret = UNKNOWN_ERROR;
        }

        if (0 <= new_slow_ae) {
            if (mSecCamera->setSlowAE(new_slow_ae) < 0) {
                ALOGV("%s::mSecCamera->setSlowAE(%d) fail", __func__, new_slow_ae);
                ret = UNKNOWN_ERROR;
            }
        }
    }
...
status_t CameraHardwareSec::sendCommand(int32_t command, int32_t arg1, int32_t arg2)
{
    ...
    if(command == CAMERA_CMD_START_FACE_DETECTION) {
        if (mSecCamera->setFaceDetect(FACE_DETECTION_ON) < 0) {
            ALOGV("ERR(%s): Fail on mSecCamera->startFaceDetection()");
            return BAD_VALUE;
        } else {
            return NO_ERROR;
        }
    ...
    if(command == CAMERA_CMD_STOP_FACE_DETECTION) {
        if (mSecCamera->setFaceDetect(FACE_DETECTION_OFF) < 0) {
            ALOGV("ERR(%s): Fail on mSecCamera->stopFaceDetection()");
            return BAD_VALUE;
        } else {
            return NO_ERROR;
        }
...
static int HAL_getNumberOfCameras()
{
    ...
    cam_fd = open(CAMERA_DEV_NAME, O_RDWR);
    if (cam_fd < 0) {
